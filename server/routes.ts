...
<div style="max-width:580px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 28px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">Lead Depot v20.7.36 — Brothers Group · Momentum Realty</div>
</div></body></html>`;
...
<div style="max-width:620px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:14px 32px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444;display:flex;justify-content:space-between">
    <span>Lead Depot v20.7.36 — Brothers Group · Momentum Realty</span>
  </div>
</div>
</body>
</html>`;
...
<div style="max-width:500px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 26px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">Lead Depot v20.7.36 — Brothers Group · Momentum Realty</div>
</div></body></html>`,
    });
...
export function registerRoutes(httpServer: ReturnType<typeof createServer>, app: Express) {
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family:Georgia,serif;background:#09090b;color:#e5e5e5;padding:40px 24px;max-width:600px;margin:0 auto;border-radius:12px;">
              ...
              <p style="color:#71717a;font-size:12px;margin-top:24px;">— Brothers Group Real Estate Team at Momentum Realty<br>Lead Depot v20.7.36</p>
            </div>
          `,
        });
      } catch (e: any) {
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family:'Georgia',serif;background:#09090b;color:#e5e5e5;padding:40px 24px;max-width:600px;margin:0 auto;border-radius:12px;">
              ...
              <p style="color:rgba(255,255,255,0.25);font-size:12px;line-height:1.6;border-top:1px solid rgba(200,170,90,0.1);padding-top:18px;">If you weren't expecting this reset, ignore this email — your password will not change. Lead Depot v20.7.36 · Brothers Group Real Estate Team at Momentum Realty</p>
            </div>
          `,
        });
        emailSent = true;
  ...
  // v20.7.36 — Admin manual bonus award. Cookie-authed via requireAdmin.
  ...
  // (e.g. an agent posted a video before the v20.7.36 toggle shipped).
  app.post("/api/admin/agents/:id/award-bonus", (req, res) => {
    ...
    console.log(`[v20.7.36 award-bonus] agent=${agentId} (${agent.name}) pts=${points >= 0 ? "+" : ""}${points} reason="${reason}"`);
...
This template is for informational/outreach purposes only.`;
  ...
  app.get("/api/admin/leaderboard", (req, res) => {
    ...
    // v20.7.36 — INV column: agent invites sent (no approval:* twin, invite fires immediately).
  ...
  app.get("/api/agent/leaderboard", (req, res) => {
    ...
    // v20.7.36 — INV column = agent invites sent. Uses direct reason match
    ...
    const win = (r: any, p: "today" | "week" | "month", agentId: number, refMap: Record<number, number>, ptsMap: Record<number, number>, ohMap: Record<number, number>, dmMap: Record<number, number>, dkMap: Record<number, number>, socMap: Record<number, number>, invMap: Record<number, number>) => {
      ...
      return {
        ...
        // v20.7.36 — INV column: agent invites sent.
  ...
  // v20.7.36 — Thresholds halved and floor rebased to $0. Payout split
  ...
  app.get("/api/team-pot", (req, res) => {
    ...
    // v20.7.36 — Month starts at $0. Team must book its first appointment to
    ...
    // v20.7.36 — Champion's Bonus fully deleted. Winner-take-all now IS the
  ...
  app.post("/api/leads/network", (req, res) => {
    ...
    // v20.7.36 — Award points AND write a lead_activity row matching the warm-
...
<div style="max-width:580px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 28px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">
    Lead Depot v20.7.36 \u2014 Brothers Group \u00b7 Momentum Realty
  </div>
</div></body></html>`,
  ...
  // v20.7.36 — Video is a WHOLE-LOG BONUS added on top of platform points, not
  ...
  app.post("/api/lead-gen/social-post", (req, res) => {
    ...
    // v20.7.36 — normalize video flag. Preferred client field is isVideoLog
    ...
    // v20.7.36 — scoring: (10 × platforms) + (isVideoLog ? 80 : 0).
    ...
    const payloadObj = {
      ...
      // v20.7.36 — whole-log video bonus. isVideoLog=true → +80 flat on top of
    ...
    broadcast({
      ...
      event: {
        ...
        platforms: platArr,
        pointsPotential,
        // v20.7.36 — whole-log video flag so admin queue can badge video posts.
        isVideo: videoLog,
        isVideoLog: videoLog,
        ts: now,
      },
  ...
  app.post("/api/candidates/invite", async (req: any, res) => {
    ...
    try {
      ...
      // v20.7.36 — Pull inviter's phone so the candidate email is signed correctly.
      ...
      // v20.7.36 — Auto-send branded invite to the candidate (if email provided).
      let emailSent = false;
      if (resend && email) {
        const candidateHtml = `
<!DOCTYPE html><html><body style="margin:0;padding:0;background:#f4f2ea;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;color:#1a1a1a;">
  ...
  // v20.7.36 ── Agent-side: list MY invitees (candidates I invited). Powers the
  ...
  // v20.7.36 ── Agent-side: nudge a still-`invited` candidate. Auto-fires a
...
<div style="max-width:680px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid rgba(200,170,90,0.25)">
  ...
  <div style="padding:16px 24px;margin-top:24px;background:#080808;border-top:1px solid rgba(255,255,255,0.05);font-size:11px;color:rgba(255,255,255,0.18);display:flex;justify-content:space-between">
    <span>Lead Depot v20.7.36</span><span>Brothers Group · Momentum Realty</span>
  </div>
</div>
</body>
</html>`;
