...
import { registerLaborCrewsRoutes } from "./laborCrews";
import { registerLaborOrdersRoutes } from "./laborOrders";
...
<div style="max-width:580px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 28px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">Lead Depot v20.57.5 — Brothers Group · Momentum Realty</div>
</div></body></html>`;
...
<div style="max-width:620px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:14px 32px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444;display:flex;justify-content:space-between">
    <span>Lead Depot v20.57.5 — Brothers Group · Momentum Realty</span>
  </div>
</div>
</body>
</html>`;
...
<div style="max-width:500px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 26px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">Lead Depot v20.57.5 — Brothers Group · Momentum Realty</div>
</div></body></html>`,
    });
...
export function registerRoutes(httpServer: ReturnType<typeof createServer>, app: Express) {
  ...
  registerLaborCrewsRoutes(app);
  registerLaborOrdersRoutes(app);
  registerBghsPublicRoutes(app);
  registerPublicAgentLandingRoutes(app); // v20.35.1 — per-agent Get In Touch landing pages
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family:Georgia,serif;background:#09090b;color:#e5e5e5;padding:40px 24px;max-width:600px;margin:0 auto;border-radius:12px;">
              ...
              <p style="color:#71717a;font-size:12px;margin-top:24px;">— Brothers Group Real Estate Team at Momentum Realty<br>Lead Depot v20.57.5</p>
            </div>
          `,
        });
      } catch (e: any) {
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family:'Georgia',serif;background:#09090b;color:#e5e5e5;padding:40px 24px;max-width:600px;margin:0 auto;border-radius:12px;">
              ...
              <p style="color:rgba(255,255,255,0.25);font-size:12px;line-height:1.6;border-top:1px solid rgba(200,170,90,0.1);padding-top:18px;">If you weren't expecting this reset, ignore this email — your password will not change. Lead Depot v20.57.5 · Brothers Group Real Estate Team at Momentum Realty</p>
            </div>
          `,
        });
        emailSent = true;
...
This template is for informational/outreach purposes only.`;
  ...
  app.post("/api/leads/network", (req, res) => {
    ...
    // v20.57.5 — Co-listed Open House Lead: award the co-host 20 pts + a
      ...
      if (parsedCohost && parsedCohost !== submitterAgentId) {
        ...
        if (coAgent && coAgent.isActive) {
          try {
            ...
            console.warn("[v20.57.2] OH-Lead co-host award failed", { leadId: created.id, cohostId: parsedCohost, err });
          }
        }
      }
    }
...
<div style="max-width:580px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 28px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">
    Lead Depot v20.57.5 \u2014 Brothers Group \u00b7 Momentum Realty
  </div>
</div></body></html>`,
  ...
  // ─── v20.50.2 PAST CLIENT APPT — FUB contact typeahead ────────────────────
  // Used by the gold "Past Client Appt" bubble's name field.
  // NOTE (v20.50.2 fix): FUB's /v1/people endpoint does NOT support a
  ...
  app.get("/api/fub/search-contacts", async (req, res) => {
    ...
    try {
      ...
      // v20.57.5 — request addresses explicitly (FUB omits them by default).
      const r = await fubRequest("GET", `/people?${param}&limit=8&fields=id,firstName,lastName,name,phones,emails,addresses`);
      const people: any[] = r.data?.people || [];
      // v20.57.5 — also surface the person's primary FUB address so the Past
      // Client Appt form can pre-fill Property Address without the agent
      // retyping it. FUB stores addresses as an array of {street, city, state,
      // code, type}; prefer type="home" but fall back to whatever is first.
      const contacts = people.map((p: any) => {
        const addrs: any[] = Array.isArray(p.addresses) ? p.addresses : [];
        const primary = addrs.find((a: any) => (a.type || "").toLowerCase() === "home") || addrs[0] || null;
        const address = primary
          ? [primary.street, primary.city, primary.state].filter(Boolean).join(", ") +
  ...
  // ─── v20.50.2 PAST CLIENT APPT — score an Appt Set from a direct past-client
  ...
  // gated to 1 per FUB contact per 30 days via the past_client_appts table.
  ...
  app.post("/api/past-client-appt", async (req, res) => {
    ...
    // 30-day anti-farm gate — 1 scored past-client appt per FUB contact.
    const recent = rawDb.prepare(`
      SELECT id, created_at FROM past_client_appts
      WHERE fub_person_id = ? AND created_at >= datetime('now', '-30 days')
      ORDER BY created_at DESC LIMIT 1
    `).get(personId) as any;
    if (recent) {
      return res.status(409).json({
        error: "This client already had a Past Client Appt scored within the last 30 days.",
  ...
  app.post("/api/lead-gen/open-house-log", async (req, res) => {
    ...
    // v20.57.5 — Co-listed OH. If a co-host was picked, verify the agent id is
    ...
    const payloadObj = {
      ...
      results,
      // v20.57.5 — Persist validated co-host on the approval row. Consumed by
  ...
  app.post("/api/admin/approvals/:id/approve", (req, res) => {
    ...
    // v20.57.5 — Co-listed Open House Log: award the co-host in parallel. Same
    ...
    if (row.kind === "open_house_log" && payload.cohostAgentId && Number(payload.cohostAgentId) !== Number(row.agent_id)) {
      ...
      try {
        ...
        console.warn("[v20.57.2] OH log co-host award failed", { requestId: id, cohostId, err: e });
      }
    }
  ...
  // v20.32.40 — Master "Ecosystem" hub page. Public, no auth. Maps the four
  // business hubs (Real Estate Team w/ nested Careers, Lead Depot, Home
  // Solutions) with a live Photo Roll fed by /api/photo-roll below.
  app.get("/ecosystem", (_req, res) => {
    const distPath = path.resolve(__dirname, "public");
    res.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
    res.sendFile(path.resolve(distPath, "ecosystem.html"));
  });

  // v20.35.1 — Item 25: Brothers Group Home Solutions public quote-request
  ...
  // Pulls the most recent APPROVED approval_requests rows that carry photo
  // evidence (open_house_log, oh_knock_route, social_post — anything with
  // photoDataUrl / photoDataUrls in payload_json) so the ecosystem page
  // auto-fills with real team content as soon as Alex/Nate approve a
  // submission in Lead Depot. No auth — these are already-approved,
  // already-public-facing marketing photos. Skips any (approval_id,
  // photo_index) pair an admin has hidden via /api/admin/company-cam.
  app.get("/api/company-cam", (_req, res) => {
    try {
      const rows = rawDb.prepare(`
        SELECT id, kind, agent_name, payload_json, decided_at, submitted_at
        FROM approval_requests
      ...
      const hiddenKeys = new Set(
        (rawDb.prepare(`SELECT approval_id, photo_index FROM company_cam_hidden`).all() as any[])
          .map((h) => `${h.approval_id}:${h.photo_index}`)
      );
      const photos: { dataUrl: string; agentName: string; kind: string; capturedAt: string }[] = [];
      for (const r of rows) {
        if (photos.length >= 16) break;
        let payload: any;
        try { payload = JSON.parse(r.payload_json); } catch { continue; }
        const urls: string[] = [];
        if (Array.isArray(payload?.photoDataUrls)) urls.push(...payload.photoDataUrls);
        else if (payload?.photoDataUrl) urls.push(payload.photoDataUrl);
        for (let idx = 0; idx < urls.length; idx++) {
          if (photos.length >= 16) break;
          const url = urls[idx];
          if (hiddenKeys.has(`${r.id}:${idx}`)) continue;
          if (typeof url === "string" && url.startsWith("data:image")) {
            photos.push({
              dataUrl: url,
              agentName: r.agent_name,
              kind: r.kind,
              capturedAt: r.decided_at || r.submitted_at,
            });
          }
        }
      }
      ...
      console.error("[company-cam]", e);
      res.status(200).json({ photos: [] });
    }
  });

  // v20.32.42 — Admin Company Cam moderation. Lists every flattened photo
  // from approved submissions (including already-hidden ones, flagged) so
  // Alex/Nate can remove not-so-great photos from the public feed without
  // touching the underlying approval record (points/audit trail intact).
  app.get("/api/admin/company-cam", (req, res) => {
    if (!requireAdmin(req, res)) return;
    try {
      const rows = rawDb.prepare(`
        SELECT id, kind, agent_name, payload_json, decided_at, submitted_at
      ...
      const photos: { approvalId: number; photoIndex: number; dataUrl: string; agentName: string; kind: string; capturedAt: string; hidden: boolean }[] = [];
      for (const r of rows) {
        let payload: any;
        try { payload = JSON.parse(r.payload_json); } catch { continue; }
        const urls: string[] = [];
        ...
        for (let idx = 0; idx < urls.length; idx++) {
          ...
          if (typeof url === "string" && url.startsWith("data:image")) {
            photos.push({
              ...
              kind: r.kind,
              capturedAt: r.decided_at || r.submitted_at,
              hidden: hiddenKeys.has(`${r.id}:${idx}`),
            });
          }
        }
      }
      ...
      console.error("[admin/company-cam]", e);
      res.status(500).json({ error: "Failed to load Company Cam photos" });
    }
  });

  // v20.32.42 — Hide/unhide a single Company Cam photo. Body: { hidden: boolean }.
  app.post("/api/admin/company-cam/:approvalId/:photoIndex", (req, res) => {
    if (!requireAdmin(req, res)) return;
    try {
      ...
      if (hidden) {
        rawDb.prepare(`
          INSERT INTO company_cam_hidden (approval_id, photo_index, hidden_by)
          VALUES (?, ?, ?)
          ON CONFLICT(approval_id, photo_index) DO NOTHING
        `).run(approvalId, photoIndex, req.currentAgent?.id ?? null);
      } else {
        rawDb.prepare(`DELETE FROM company_cam_hidden WHERE approval_id = ? AND photo_index = ?`).run(approvalId, photoIndex);
      }
      res.json({ ok: true, hidden });
    } catch (e) {
      console.error("[admin/company-cam hide]", e);
  ...
  // v20.35.1 — Per-agent public "Get In Touch" landing page (QR code destination
...
<div style="max-width:680px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid rgba(200,170,90,0.25)">
  ...
  <div style="padding:16px 24px;margin-top:24px;background:#080808;border-top:1px solid rgba(255,255,255,0.05);font-size:11px;color:rgba(255,255,255,0.18);display:flex;justify-content:space-between">
    <span>Lead Depot v20.57.5</span><span>Brothers Group · Momentum Realty</span>
  </div>
</div>
</body>
</html>`;
      ...
      try {
        if (typeof (globalThis as any).sendEmailToAdmins === "function") {
          ...
          const resp = await fetch("https://api.resend.com/emails", {
            ...
            body: JSON.stringify({
              from: "Lead Depot <noreply@watsonbrothersgroup.com>", // v20.35.1 — item 28 audit: was missing a display name
              to: ["alex@watsonbrothersgroup.com", "nate@watsonbrothersgroup.com"],
              subject,
              text: bodyLines.join("\n"),
            }),
