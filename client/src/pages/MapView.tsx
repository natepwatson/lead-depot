...
interface GeocodedLead extends MapLead { lat: number; lng: number; }
...
const STATUS: Record<string, { fill: string; label: string }> = {
  ...
  left_voicemail:           { fill: "#9a7fc7", label: "Voicemail" },
...
const gc: Record<string, { lat: number; lng: number } | null> = {};
...
export default function MapView() {
  ...
  const [geo, setGeo] = useState<GeocodedLead[]>([]);
  ...
  useEffect(() => { loadLF().then(() => setLfReady(true)).catch(() => setErr("Failed to load map library.")); }, []);
  useEffect(() => {
    fetch("/api/leads/map").then(r => r.json()).then((d: MapLead[]) => { setLeads(d); setLoading(false); }).catch(() => { setErr("Failed to load leads."); setLoading(false); });
  }, []);
  useEffect(() => {
    if (!lfReady || loading || leads.length === 0) return;
    setGeocoding(true); setTot(leads.length);
    (async () => {
      const res: GeocodedLead[] = [];
      for (let i = 0; i < leads.length; i++) {
        const l = leads[i]; const c = await geocode(l.address, l.city, l.state, l.zip);
        if (c) res.push({ ...l, ...c });
        setProg(i + 1);
      }
      setGeo(res); setGeocoding(false);
    })();
  }, [lfReady, loading, leads]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const list = filter === "all" ? geo : geo.filter(l => l.status === filter);
    list.forEach(l => {
      const popup = `<div style="background:#0f0f0f;border:1px solid rgba(200,170,90,0.2);border-radius:6px;padding:10px 14px;min-width:190px;font-family:'Switzer','Inter',sans-serif;font-size:11px;line-height:1.6;color:#e8e4dc">${l.ownerName ? `<b style="color:#c8aa5a;font-size:12px">${l.ownerName}</b><br/>` : ""}<span style="color:#797876">${l.address}${l.city ? ", " + l.city : ""}</span><br/><span style="display:inline-block;margin-top:5px;padding:2px 8px;border-radius:3px;background:rgba(200,170,90,0.1);color:#c8aa5a;font-size:10px;letter-spacing:.08em;text-transform:uppercase">${label(l.status)}</span><span style="color:#555;font-size:10px;margin-left:6px">${l.leadType}</span></div>`;
      L.marker([l.lat, l.lng], { icon: pin(color(l.status), L) }).bindPopup(popup, { className: "bgre-popup", maxWidth: 270, offset: [0, -6] }).addTo(layerRef.current);
    });
  }, [geo, filter]);

  const counts = geo.reduce<Record<string, number>>((a, l) => { a[l.status] = (a[l.status] ?? 0) + 1; return a; }, {});
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <div style={{ padding: "5px 12px", background: "rgba(200,170,90,0.08)", border: "1px solid rgba(200,170,90,0.2)", borderRadius: 20, fontSize: 11, color: "#c8aa5a" }}>{leads.length} total leads</div>
          <div style={{ padding: "5px 12px", background: "rgba(79,152,163,0.08)", border: "1px solid rgba(79,152,163,0.2)", borderRadius: 20, fontSize: 11, color: "#4f98a3" }}>{geo.length} mapped</div>
          {geocoding && <div style={{ padding: "5px 12px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, fontSize: 11, color: "#797876" }}>Locating {prog}/{tot}…</div>}
        </div>
      </div>
      <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
        {[{ v: "all", l: "All", c: "#c8aa5a", n: geo.length }, ...Object.entries(STATUS).map(([v, { label: l, fill: c }]) => ({ v, l, c, n: counts[v] ?? 0 }))].filter(o => o.v === "all" || o.n > 0).map(o => {
          const active = filter === o.v;
          return <button key={o.v} onClick={() => setFilter(o.v)} style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 10px", background: active ? `${o.c}20` : "rgba(255,255,255,0.03)", border: `1px solid ${active ? o.c + "55" : "rgba(255,255,255,0.07)"}`, borderRadius: 16, cursor: "pointer", fontSize: 10, color: active ? o.c : "rgba(255,255,255,0.4)", letterSpacing: "0.05em", transition: "all 0.15s" }}><span style={{ width: 7, height: 7, borderRadius: "50%", background: o.c, flexShrink: 0 }} />{o.l} <span style={{ opacity: 0.65 }}>{o.n}</span></button>;
        })}
      </div>
      <div style={{ height: "calc(100vh - 340px)", minHeight: 440, position: "relative", borderRadius: 10, overflow: "hidden", border: "1px solid rgba(200,170,90,0.15)" }}>
        ...
        {(loading || !lfReady) && !err && (
          <div style={{ position: "absolute", inset: 0, background: "#080808", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, zIndex: 1000 }}>
            <svg width="36" height="36" viewBox="0 0 36 36" fill="none"><rect x="2" y="18" width="32" height="15" rx="1" stroke="#c8aa5a" strokeWidth="1.4"/><path d="M2 18 L18 5 L34 18" stroke="#c8aa5a" strokeWidth="1.4" strokeLinejoin="round" fill="none"/><rect x="13" y="24" width="10" height="9" rx="0.5" stroke="#c8aa5a" strokeWidth="1.2"/></svg>
            <p style={{ fontSize: 11, color: "rgba(200,170,90,0.5)", letterSpacing: "0.14em" }}>LOADING MAP…</p>
          </div>
        )}
        {err && <div style={{ position: "absolute", inset: 0, background: "#080808", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}><p style={{ fontSize: 12, color: "#dd6974" }}>{err}</p></div>}
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, padding: "10px 14px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 8 }}>
        ...
        {Object.entries(STATUS).map(([s, { fill: f, label: l }]) => (
          <div key={s} style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ width: 8, height: 8, borderRadius: "50%", background: f }} /><span style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", letterSpacing: "0.04em" }}>{l}</span></div>
        ))}
      </div>
