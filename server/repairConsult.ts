...
const IN_HOUSE_ITEMS: SeedItem[] = [
  ...
  { key: "paint_ext_body", category: "in_house", trade: "painting_exterior", name: "Exterior Painting — Body", unit: "sqft", rate: 2.25, min: 800, twoStory: true, seq: 40, instruction: "Paint exterior body — {qty} sqft. Color-matched to existing.{story}", notes: "Color match is visual-sample only, not guaranteed; slight sheen/tone variance vs. original is possible." },
  { key: "paint_ext_trim", category: "in_house", trade: "painting_exterior", name: "Exterior Painting — Trim & Doors", unit: "linear_ft", rate: 3.50, min: 150, twoStory: true, seq: 41, instruction: "Paint exterior trim & doors — {qty} linear ft. Color-matched to existing.{story}", notes: "Color match is visual-sample only, not guaranteed; slight sheen/tone variance vs. original is possible." },
  ...
  { key: "paint_int_body", category: "in_house", trade: "painting_interior", name: "Interior Painting — Body (Walls)", unit: "sqft", rate: 2.00, min: 600, seq: 55, instruction: "Paint interior walls — {qty} sqft. Color-matched to existing.", notes: "Color match is visual-sample only, not guaranteed; slight sheen/tone variance vs. original is possible." },
  { key: "paint_int_trim", category: "in_house", trade: "painting_interior", name: "Interior Painting — Trim & Doors", unit: "linear_ft", rate: 3.00, min: 150, seq: 56, instruction: "Paint interior trim & doors — {qty} linear ft. Color-matched to existing.", notes: "Color match is visual-sample only, not guaranteed; slight sheen/tone variance vs. original is possible." },
  { key: "paint_int_ceiling", category: "in_house", trade: "painting_interior", name: "Interior Painting — Ceiling", unit: "sqft", rate: 1.75, min: 300, seq: 57, instruction: "Paint ceiling — {qty} sqft. Color-matched to existing (standard flat white/existing ceiling color).", notes: "Color match is visual-sample only, not guaranteed; slight sheen/tone variance vs. original is possible." },
  ...
  // v20.33.1 — Pricing model revised per Alex: parts/fixtures/appliances are
  // sold to the client AT COST (readily-available Home Depot/Lowe's stock
  // items; custom/special-order units carry an additional expense beyond
  // that cost). Brothers Group's margin comes from separate, itemized fees:
  // a delivery/procurement fee (shared "materials_pickup_delivery" item,
  // added once per job when we handle pickup/delivery) and a per-fixture
  // installation-labor fee (the rate below on each item — labor ONLY, no
  // material markup baked in). Every item's liability is explicitly limited
