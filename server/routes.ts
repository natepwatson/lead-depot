...
This template is for informational/outreach purposes only.`;
  ...
  app.post("/api/leads/network", (req, res) => {
    ...
    // v20.58.4 — Co-listed Open House Lead: award the co-host 20 pts + a
...
<div style="max-width:580px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  app.get("/api/fub/search-contacts", async (req, res) => {
    ...
    try {
      ...
      // v20.58.4 — request addresses explicitly (FUB omits them by default).
      ...
      // v20.58.4 — also surface the person's primary FUB address so the Past
  ...
  app.post("/api/lead-gen/open-house-log", async (req, res) => {
    ...
    // v20.58.4 — Co-listed OH. If a co-host was picked, verify the agent id is
    ...
    const payloadObj = {
      ...
      results,
      // v20.58.4 — Persist validated co-host on the approval row. Consumed by
  ...
  app.post("/api/admin/approvals/:id/approve", (req, res) => {
    ...
    // v20.58.4 — Co-listed Open House Log: award the co-host in parallel. Same
