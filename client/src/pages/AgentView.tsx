...
function ClientReferralForm(props: { source?: WarmLeadSource; addressPrefill?: string; onSubmitted?: (leadId: number) => void } = {}) {
  ...
  // v20.58.4 — Co-listed OH: only shown when source === "open_house". Awards 20 pts to co-host instantly on submit.
  ...
  // v20.58.4 — Lazy-load active agent list once the co-host toggle is flipped.
  useEffect(() => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const r = await apiRequest("POST", "/api/leads/network", {
        ...
        // v20.58.4 — Co-listed OH Lead. Server awards co-host 20 pts + activity row (open_house_lead_cohost).
  ...
  return (
    <div style={{
      ...
      <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {/* v20.58.4 — Co-listed OH: only shown when source is open_house. Both agents get 20 pts on submit. */}
        {isOpenHouseLead && (
          <div style={{
            padding: "10px 12px", borderRadius: 8,
...
function PastClientApptForm(props: { user: any; toast: any; onDone: () => void }) {
  ...
  const pickContact = (c: { id: number; name: string; phone: string; email: string; address?: string }) => {
    ...
    setQuery(c.name);
    setResults([]);
    // v20.58.4 — auto-fill Property Address from FUB when the picked contact
...
function OpenHouseLogForm(props: { user: any; toast: any; onDone: () => void }) {
  ...
  // v20.58.4 — Co-listed OH: agent can flag this OH as co-hosted with another
  ...
  const submit = async () => {
    ...
    try {
      const r = await apiRequest("POST", "/api/lead-gen/open-house-log", {
        ...
        // v20.58.4 — Co-listed OH. Server persists these on the approval row.
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{
        ...
        {/* v20.58.4 — Co-listed open house. Both agents get the full 50 pts on approval. */}
