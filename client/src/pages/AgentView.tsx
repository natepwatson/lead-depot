...
export function TeamPotCard() {
  ...
  const serverLadder: Array<{ tier: number; appts: number; pot: number }> = Array.isArray(pot.ladder) && pot.ladder.length > 0
    ? pot.ladder
    : (serverNextTier
        ? [
            { tier: serverCurrentTier.tier, appts: serverCurrentTier.appts, pot: serverCurrentTier.pot },
            { tier: serverNextTier.tier,    appts: serverNextTier.appts,    pot: serverNextTier.pot },
          ]
        : [{ tier: serverCurrentTier.tier, appts: serverCurrentTier.appts, pot: serverCurrentTier.pot }]);
  const rungs: Rung[] = serverLadder
    .slice()
    ...
    .map(r => ({
      ...
      pot: r.pot,
      label: `$${r.pot}`,
      mystery: nextTierMystery && serverNextTier != null && r.tier === serverNextTier.tier,
    }));

  // Progress: fill the meter proportionally across the WHOLE ladder so the fill
  // lines up with the position of each rung. When the team is at 4 appts on a
  // 0/5/10/15 ladder we want the fill to be 4/15 = ~27% across the bar and land
  // just before the $500 rung, not run to the far right.
  const barStart = rungs[0].appts;
  const barEnd = rungs[rungs.length - 1].appts;
  const barSpan = Math.max(1, barEnd - barStart);
  const progressAppts = Math.max(barStart, Math.min(barEnd, teamAppts));
  const progressPct = serverNextTier
    ? Math.min(100, ((progressAppts - barStart) / barSpan) * 100)
    : 100;
  const rungPct = (r: Rung): number => {
    if (r.appts <= barStart) return 0;
    if (r.appts >= barEnd) return 100;
    return ((r.appts - barStart) / barSpan) * 100;
  };
...
function ChallengesTab() {
  ...
  useEffect(() => {
    if (!claimOpen && !unlockOpen) return;
    document.body.classList.add("ld-modal-open");
    return () => document.body.classList.remove("ld-modal-open");
  }, [claimOpen, unlockOpen]);
...
function ClientReferralForm(props: { source?: WarmLeadSource; addressPrefill?: string; onSubmitted?: (leadId: number) => void } = {}) {
  ...
  // v20.58.0 — Co-listed OH: only shown when source === "open_house". Awards 20 pts to co-host instantly on submit.
  ...
  // v20.58.0 — Lazy-load active agent list once the co-host toggle is flipped.
  useEffect(() => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const r = await apiRequest("POST", "/api/leads/network", {
        ...
        // v20.58.0 — Co-listed OH Lead. Server awards co-host 20 pts + activity row (open_house_lead_cohost).
  ...
  return (
    <div style={{
      ...
      <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {/* v20.58.0 — Co-listed OH: only shown when source is open_house. Both agents get 20 pts on submit. */}
        {isOpenHouseLead && (
          <div style={{
            padding: "10px 12px", borderRadius: 8,
...
function PastClientApptForm(props: { user: any; toast: any; onDone: () => void }) {
  ...
  const pickContact = (c: { id: number; name: string; phone: string; email: string; address?: string }) => {
    ...
    setQuery(c.name);
    setResults([]);
    // v20.58.0 — auto-fill Property Address from FUB when the picked contact
...
export default function AgentView({ onBackToAdmin, onOpenAdmin, initialTab, mode = "seller" }: { onBackToAdmin?: () => void; onOpenAdmin?: () => void; initialTab?: Tab; mode?: "seller" } = {}) {
  ...
  const [listingRepairPrefill, setListingRepairPrefill] = useState<{ address: string; name: string; email: string; phone: string; heroPhotoUrl?: string | null; galleryUrls?: string[]; flaggedPillars?: { key: string; label: string; tier: string; notes: string; details?: string[] }[] } | null>(null);
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
              ...
              <span style={{ fontSize: 9, color: "rgba(200,170,90,0.55)", letterSpacing: "0.10em", fontWeight: 700 }}>v20.58.0</span>
            </div>
          </div>
          {onBackToAdmin && (
...
function OpenHouseLogForm(props: { user: any; toast: any; onDone: () => void }) {
  ...
  // v20.58.0 — Co-listed OH: agent can flag this OH as co-hosted with another
  ...
  const submit = async () => {
    ...
    try {
      const r = await apiRequest("POST", "/api/lead-gen/open-house-log", {
        ...
        // v20.58.0 — Co-listed OH. Server persists these on the approval row.
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{
        ...
        {/* v20.58.0 — Co-listed open house. Both agents get the full 50 pts on approval. */}
