...
export function ensureInspectionsSchema() {
  ...
  // v20.32.33 — tracks whether the one-time "you still haven't wired us"
...
// v20.32.33 — GAP FIX: signing an add-on previously only fired an internal
...
// v20.32.33 — GAP FIX: previously, a client declining an order updated the DB
...
export function registerInspectionsRoutes(app: Express) {
  ...
  app.post("/api/inspection-order/:token/accept", async (req: any, res: Response) => {
    ...
    // v20.32.33 — separate milestone specifically for chasing the WIRE (not
  ...
  app.post("/api/inspection-order/:token/decline", async (req: any, res: Response) => {
    ...
    // v20.32.33 — GAP FIX: nobody used to hear about this. Notify admins now.
  ...
  app.post("/api/inspection-addon/:token/sign", async (req: any, res: Response) => {
    ...
    try {
      ...
      // v20.32.33 — GAP FIX: this used to be the ONLY email fired on add-on
  ...
  app.get("/api/admin/inspection-orders", (req: any, res: Response) => {
    if (!req.currentAgent || req.currentAgent.role !== "admin") return res.status(403).json({ error: "Admin only" });
    // v20.32.33 — GAP FIX: admin table previously showed only the order total,
  ...
  app.post("/api/admin/inspection-orders/:id/complete", (req: any, res: Response) => {
    ...
    // v20.32.33 — REMOVED the "invoice_sent" milestone that used to fire here.
  ...
  // v20.32.33 — GAP FIX: automatic one-time reminder for orders the client
