...
// v16.7 — Daily metrics snapshots: forever-log of every agent's full metrics
...
// v16.7 — LEAD_ACTIVITY DIAL-COUNT PRESERVATION.
//
// Before v16.7, when a lead's last phone was Disconnected or Wrong#'d, the
...
// Snapshot columns are populated in routes.ts at insert time (v16.7+ writes).
try {
  ...
  if (leadIdCol && leadIdCol.notnull === 1) {
    console.log('[v16.7] Rebuilding lead_activity to make lead_id nullable…');
    rawDb.exec(`
      BEGIN;
      ...
      COMMIT;
    `);
    console.log('[v16.7] lead_activity rebuilt: lead_id now nullable, ON DELETE SET NULL');
  }
} catch (err) {
...
try {
  ...
  const names = new Set(cols.map(c => c.name));
  if (!names.has("source"))       rawDb.exec(`ALTER TABLE listings ADD COLUMN source TEXT DEFAULT 'excel'`);       // 'excel' | 'fub:{tag}' | 'manual'
  if (!names.has("source_ref"))   rawDb.exec(`ALTER TABLE listings ADD COLUMN source_ref TEXT`);                   // FUB person id, workbook row #, etc
  if (!names.has("days_on_market")) rawDb.exec(`ALTER TABLE listings ADD COLUMN days_on_market INTEGER`);
  if (!names.has("beds"))         rawDb.exec(`ALTER TABLE listings ADD COLUMN beds INTEGER`);
  if (!names.has("baths"))        rawDb.exec(`ALTER TABLE listings ADD COLUMN baths REAL`);
...
rawDb.exec(`
  CREATE TABLE IF NOT EXISTS buyers (
    ...
    phone                 TEXT,
    email                 TEXT,
    buyers_agent          TEXT,                        -- team member owning this buyer
    status                TEXT NOT NULL DEFAULT 'active', -- 'active' | 'closed' | 'paused'
    price_min             INTEGER,
    price_max             INTEGER,
    preferred_areas       TEXT,                        -- csv "Nocatee,Ponte Vedra,32082"
    beds_min              INTEGER,
    baths_min             REAL,
    sqft_min              INTEGER,
    must_haves            TEXT,
    no_gos                TEXT,
    pre_approved          INTEGER DEFAULT 0,           -- 0/1
    pre_approval_amount   INTEGER,
    lender                TEXT,
    timeline              TEXT,                        -- '0-30' | '30-60' | '60-90' | '90+'
    closed_date           TEXT,
    closed_address        TEXT,
    closed_price          INTEGER,
    notes                 TEXT,
    source                TEXT DEFAULT 'excel',        -- 'excel' | 'fub:{tag}' | 'manual'
    source_ref            TEXT,
    fub_person_id         TEXT,
...
rawDb.exec(`
  CREATE TABLE IF NOT EXISTS fub_tag_config (
    ...
    bucket                TEXT NOT NULL,               -- 'pocket_listing' | 'active_buyer' | 'ignore'
    enabled               INTEGER NOT NULL DEFAULT 1,
    last_synced_at        TEXT,
    last_person_count     INTEGER DEFAULT 0,
    created_at            TEXT NOT NULL DEFAULT (datetime('now')),
    updated_at            TEXT NOT NULL DEFAULT (datetime('now'))
  )
`);
...
`);
console.log("[db] v20.6.7 newsletter_inputs table ready");

// ─── v20.6.7 one-shot buyer master-list cleanup ────────────────────────────
...
try {
  ...
  if (delObj.changes > 0) console.log(`[db] v20.6.7 cleanup: deleted ${delObj.changes} '[object Object]' buyer row(s)`);
  const swapPrices = rawDb.prepare(`
    UPDATE buyers
       SET price_min = price_max,
           price_max = price_min,
           updated_at = datetime('now'),
           last_updated_by = 'v20.6.7-cleanup'
     WHERE price_min IS NOT NULL AND price_max IS NOT NULL AND price_min > price_max
  `).run();
  if (swapPrices.changes > 0) console.log(`[db] v20.6.7 cleanup: swapped inverted price ranges on ${swapPrices.changes} buyer row(s)`);
  ...
  `).run();
  if (zeroInvestor.changes > 0) console.log(`[db] v20.6.7 cleanup: normalized text "No" is_investor → 0 on ${zeroInvestor.changes} row(s)`);
  if (fixInvestor.changes > 0) console.log(`[db] v20.6.7 cleanup: normalized is_investor on ${fixInvestor.changes} buyer row(s)`);
} catch (e) {
  console.error("[db] v20.6.7 cleanup failed (non-fatal):", (e as any)?.message || e);
}
