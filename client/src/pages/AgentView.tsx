...
import { enqueueAndSendTap, subscribeQueueDepth } from "@/lib/tapQueue";  // v16.4
...
function LeadCard({ lead }: { lead: Lead }) {
  ...
  const outcomeMutation = useMutation({
    mutationFn: (data: { outcome: string; notes?: string; callbackDate?: string; apptEmail?: string; confirmedAddress?: string; apptDate?: string; apptTime?: string; stage?: string; intention?: string; dialedPhone?: string; followUpTiming?: string }) => {
      ...
      // v16.4 — Route through offline tap queue. Every tap gets a UUID + is
...
export function TeamPotCard() {
  ...
  // v16.4 — rescale: floor $250 (0 appts), 10 → $500, 20 → $750, 30 → $1000.
  ...
  return (
    <>
      ...
      >
        ...
        {/* v16.4 — Champion's Bonus panel. Locked until team reaches 30 appts,
...
function LeaderboardTab({ mode = "seller" }: { mode?: "seller" | "recruiting" } = {}) {
  ...
  // v16.4 — Leaderboard window tab. Server now returns per-agent .windows.today
  ...
  // v16.4 — sort by the currently selected window.
  ...
  return (
    <div style={{ width: "100%", padding: "0 0 20px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        {/* v16.4 — window tabs (Today / Week / Month / All). Matches admin leaderboard tabs. */}
            ...
            {ranked.map((s, i) => {
              ...
              return (
                <div key={s.agent.id} style={{
                  ...
                  {/* v16.4 — metrics come from the selected window (Today / Week /
