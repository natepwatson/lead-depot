  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "22px 20px 16px" }}>
        ...
        {/* v19.3 — Zillow Intel (public scrape, 24h cache). Renders inline if scrape succeeds.
...
export function TeamPotCard() {
  ...
  const { data: pot } = useQuery<any>({
    ...
    refetchInterval: 45000, // v19.3 — money pool changes slowly (WS covers real wins)
    staleTime: 5000,
  });
...
// v19.3 — HomeShell wraps LeaderboardTab with a small "Board ↔ Team Map"
...
// v19.3 — ZillowIntelPanel: lazy-loads /api/zillow/intel when a lead detail
...
function MyLeadsTab({ onOpenLead }: { onOpenLead?: (leadId: number) => void }) {
  ...
  // v19.3 — Pipeline view toggle: List (default, phone-friendly) or Kanban (6 stages).
...
// v19.3 — KanbanBoard: 6-column horizontally-scrollable board.
...
export default function AgentView({ onBackToAdmin, onOpenAdmin, initialTab, mode = "seller" }: { onBackToAdmin?: () => void; onOpenAdmin?: () => void; initialTab?: Tab; mode?: "seller" } = {}) {
  ...
  // v19.3 — Prime Time notifier boot. Idempotent; only fires when permission is granted.
  ...
  const { data: liveCountData } = useQuery<{ dialingNow: number; windowMinutes: number; lastActivityAt: string | null }>({
    ...
    refetchInterval: 60000, // v19.3 — presence badge, 60s lag imperceptible
    staleTime: 15000,
  });
  ...
  const { data: agentLeadCount } = useQuery<{ count: number }>({
    ...
    enabled: prospectingMode,
    refetchInterval: 30000, // v19.3 — prospecting counter
  });
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
              ...
              <span style={{ fontSize: 9, color: "rgba(200,170,90,0.55)", letterSpacing: "0.10em", fontWeight: 700 }}>v19.3</span>
            </div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
