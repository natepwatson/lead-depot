// ─── FREC AGENT RECRUITING PIPELINE ───────────────────────────────────────────
// Consumes FrecRecord[] from frec-scraper.ts, deduplicates against existing
// agent_leads, territory-filters to our 7 NE Florida territories, and inserts
// new recruits into the pipeline as status='new', source='frec_scrape'.
//
// Runs weekly (Sunday 2am EDT) via scheduled cron.
// Also triggerable manually via POST /api/admin/frec-run.
// ─────────────────────────────────────────────────────────────────────────────

import { createRequire } from "node:module";
import { scrapeAllFrec } from "./frec-scraper";
import { getTerritoryForZip, ALL_NE_FLORIDA_ZIPS } from "./territories";

const require = createRequire(typeof __filename !== "undefined" ? __filename : import.meta.url);

// ─── DEDUP HASH ───────────────────────────────────────────────────────────────
// Primary key: frecLicenseId (most stable)
// Fallback composite: normalized email + zip (for overlap with manually entered agents)

function makeDedupHash(frecLicenseId: string, email: string, zip: string): string {
  const crypto = require("node:crypto");
  const key = `${frecLicenseId || ""}:${(email || "").toLowerCase().trim()}:${(zip || "").trim()}`;
...
export interface FrecPipelineResult {
  scraped: number;          // total raw records from FREC
...
export async function runFrecPipeline(rawDb: any): Promise<FrecPipelineResult> {
  const startMs = Date.now();
  console.log("[FREC Pipeline] Starting weekly recruiting scrape...");

  // 1. Run full scrape across all counties × license types
  const { records: allRecords, countByCounty, errors } = await scrapeAllFrec();

  console.log(`[FREC Pipeline] Scrape complete: ${allRecords.length} raw records. By county:`, countByCounty);

  const result: FrecPipelineResult = {
  ...
  // 2. Build dedup sets from existing agent_leads (both frecLicenseId and email)
  ...
  const existingLicenseIds = new Set<string>(
    (rawDb.prepare(`SELECT frec_license_id FROM agent_leads WHERE frec_license_id IS NOT NULL`).all() as any[])
      .map((r: any) => r.frec_license_id)
  );
  ...
  const insertStmt = rawDb.prepare(`
    INSERT INTO agent_leads (
      ...
      frec_license_id, license_issue_date, license_expire_date,
  ...
  const refreshStmt = rawDb.prepare(`
    ...
    SET current_brokerage = ?,
        license_expire_date = ?,
        last_scraped_at = ?,
        frec_license_id = COALESCE(frec_license_id, ?)
    WHERE dedup_hash = ?
  `);
  ...
  for (const record of allRecords) {
    ...
    const inFootprint = territory !== null || (
      record.zip === "" && FREC_TARGET_COUNTIES.includes(record.county)
    );
    ...
    const hash = makeDedupHash(record.frecLicenseId, record.email, record.zip);
    ...
    // ── Check 2: frecLicenseId match (same agent, different hash due to email change)
    if (record.frecLicenseId && existingLicenseIds.has(record.frecLicenseId)) {
      refreshByLicenseIdStmt.run(
        record.currentBrokerage, record.licenseExpireDate, now,
        hash, record.frecLicenseId
      );
      result.updated++;
      result.skipped++;
      continue;
    ...
    try {
      ...
      if (record.frecLicenseId) existingLicenseIds.add(record.frecLicenseId);
      ...
      if (err.message?.includes("UNIQUE")) {
        ...
        console.error("[FREC Pipeline] Insert error:", err.message, record.frecLicenseId);
        result.errors.push(`insert:${record.frecLicenseId}:${err.message}`);
      }
    }
  }
  ...
  console.log(
    `[FREC Pipeline] Done. Scraped=${result.scraped} InFootprint=${result.inFootprint} ` +
