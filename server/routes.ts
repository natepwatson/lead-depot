...
<div style="max-width:620px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:14px 32px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444;display:flex;justify-content:space-between">
    <span>Lead Depot v14.79 — Brothers Group · Momentum Realty</span>
  </div>
</div>
</body>
</html>`;
...
<div style="max-width:560px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 28px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">Lead Depot v14.79 — Brothers Group · Momentum Realty</div>
</div></body></html>`;
...
<div style="max-width:500px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 26px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">Lead Depot v14.79 — Brothers Group · Momentum Realty</div>
</div></body></html>`,
    });
...
export function registerRoutes(httpServer: ReturnType<typeof createServer>, app: Express) {
  ...
  app.get("/api/me/:id", (req, res) => {
    ...
    // v14.79 — Drizzle exposes camelCase (headshotUrl, homeAddress) not snake_case.
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family:Georgia,serif;background:#09090b;color:#e5e5e5;padding:40px 24px;max-width:600px;margin:0 auto;border-radius:12px;">
              ...
              <p style="color:#71717a;font-size:12px;margin-top:24px;">— Brothers Group Real Estate Team at Momentum Realty<br>Lead Depot v14.79</p>
            </div>
          `,
        });
      } catch (e: any) {
  ...
  app.patch("/api/agents/:id/reactivate", (req, res) => {
    ...
    // v14.79 — Removed the 7-day reactivate window. Admins should be able to
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family:'Georgia',serif;background:#09090b;color:#e5e5e5;padding:40px 24px;max-width:600px;margin:0 auto;border-radius:12px;">
              ...
              <p style="color:rgba(255,255,255,0.25);font-size:12px;line-height:1.6;border-top:1px solid rgba(200,170,90,0.1);padding-top:18px;">If you weren't expecting this reset, ignore this email — your password will not change. Lead Depot v14.79 · Brothers Group Real Estate Team at Momentum Realty</p>
            </div>
          `,
        });
        emailSent = true;
  ...
  // v14.79 — Admin-only HARD DELETE. Permanently removes an agent row.
  ...
  app.get("/api/leads/my-next", (req, res) => {
    ...
    const alreadyLocked: any = rawDb.prepare(`
      ...
      LIMIT 1
    `).get(agentId);
    // v14.79 — Helper: count how many times THIS agent has dialed THIS lead
    ...
    // v14.79 — Per-agent, per-lead, per-day dial counter (see comment above where
  ...
  // ─── MY PIPELINE (restored v14.79, no date filter) ────────────────────────
  // v14.79 (moved here v14.79 hotfix) — MUST be registered BEFORE `/api/leads/:id`
  ...
  app.post("/api/leads/:id/outcome", async (req, res) => {
    ...
    if (deadOutcomes.includes(outcome)) {
      ...
      // v14.79 — Raised from 10 → 12. With higher-quality LandVoice lists we expect
      ...
      // v14.79 — `remaining` was undefined here since v14.65 refactor (would have
      ...
      // v14.65 — Raised from 6 → 10 to give more attempts to hunt the true owner
      //           before retiring a line.
      // v14.79 — Raised from 10 → 12 (see PHONE_ATTEMPT_CAP comment above).
    ...
    if (outcome === "wrong_number") {
      ...
      broadcast({ type: "lead_updated", leadId });
      // v14.79 — Same fix as Disconnected branch above: `remaining` was undefined.
      return res.json({ updated: true, leadId, nextPhone: nextViable, remaining: phones.length, keptOnLead: !!untriedNext });
    }
...
{agentPhone} \u00b7 {agentEmail}`;
  ...
  // ─── CLAIM A CALLBACK (v14.79) ────────────────────────────────────────────
  // v14.79 — Alex: "With the phone-number look-up I want anyone to grab it if
  ...
  app.post("/api/admin/seller-hard-reset", (req: any, res) => {
    ...
    try {
      ...
      const txn = rawDb.transaction(() => {
        ...
        // v14.79: lead_locks has FK to leads, must go first or DELETE FROM leads throws.
...
<div style="max-width:580px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 28px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">
    Lead Depot v14.79 \u2014 Brothers Group \u00b7 Momentum Realty
  </div>
</div></body></html>`,
...
<div style="max-width:680px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid rgba(200,170,90,0.25)">
  ...
  <div style="padding:16px 24px;margin-top:24px;background:#080808;border-top:1px solid rgba(255,255,255,0.05);font-size:11px;color:rgba(255,255,255,0.18);display:flex;justify-content:space-between">
    <span>Lead Depot v14.79</span><span>Brothers Group · Momentum Realty</span>
  </div>
</div>
</body>
</html>`;
