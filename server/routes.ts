...
function awardPoints(
  ...
  if (!agentId) return;
  // v15.11.24 — REBALANCED point system.
  ...
  const pts: Record<string, number> = {
    ...
    email_sent:                 0,   // v15.11.24 — REMOVED. Email no longer awards points.
    email_sent_value:           0,   // v15.11.24 — REMOVED. Email no longer awards points.
    left_voicemail:             0,   // v15.11.24 — REMOVED. VM no longer awards points.
    // Any other outcome falls back to base dial (1).
  };
  const basePoints = pts[outcome] ?? 1;
  // v15.11.24 — 5-TIER call-heat multiplier (was 2-tier). Multipliers align with
...
<div style="max-width:620px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:14px 32px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444;display:flex;justify-content:space-between">
    <span>Lead Depot v15.11.24 — Brothers Group · Momentum Realty</span>
  </div>
</div>
</body>
</html>`;
...
<div style="max-width:560px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 28px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">Lead Depot v15.11.24 — Brothers Group · Momentum Realty</div>
</div></body></html>`;
...
<div style="max-width:500px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 26px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">Lead Depot v15.11.24 — Brothers Group · Momentum Realty</div>
</div></body></html>`,
    });
...
export function registerRoutes(httpServer: ReturnType<typeof createServer>, app: Express) {
  ...
  // ─── v15.11.24 — Admin session-guarded set-password endpoint ───
  ...
  app.post("/api/admin/agents/:id/set-password", async (req, res) => {
    ...
    console.log(`[v15.11.24 admin-set-password] password set for agent ${id} (${row.email}) by admin ${req.currentAgent.email}`);
    res.json({ ok: true, agentId: id, email: row.email, name: row.name });
  });
  ...
  const CRON_EXEMPT_PATHS = [
    ...
    // v15.11.24 — Holdout admin endpoints. INGEST_SECRET-guarded inside route.
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family:Georgia,serif;background:#09090b;color:#e5e5e5;padding:40px 24px;max-width:600px;margin:0 auto;border-radius:12px;">
              ...
              <p style="color:#71717a;font-size:12px;margin-top:24px;">— Brothers Group Real Estate Team at Momentum Realty<br>Lead Depot v15.11.24</p>
            </div>
          `,
        });
      } catch (e: any) {
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family:'Georgia',serif;background:#09090b;color:#e5e5e5;padding:40px 24px;max-width:600px;margin:0 auto;border-radius:12px;">
              ...
              <p style="color:rgba(255,255,255,0.25);font-size:12px;line-height:1.6;border-top:1px solid rgba(200,170,90,0.1);padding-top:18px;">If you weren't expecting this reset, ignore this email — your password will not change. Lead Depot v15.11.24 · Brothers Group Real Estate Team at Momentum Realty</p>
            </div>
          `,
        });
        emailSent = true;
  ...
  // v15.11.24 — Agents can no longer self-service password changes. Admins (Alex/Nate)
  ...
  // v15.11.24 — Admin holdout clearing. Skip and Recycle both write to
  ...
  // v15.11.24 — List active holdouts, filterable by agent or lead. INGEST_SECRET guarded.
  ...
  app.get("/api/leads/my-next", (req, res) => {
    ...
    // v15.11.24 — Sweep expired agent_lead_holdouts. Cheap because the index
    // is on (agent_id, until) and there are at most a few dozen active rows
    // per agent.
    rawDb.prepare(`DELETE FROM agent_lead_holdouts WHERE until < datetime('now')`).run();

    // v15.11.24 — Guard: never resurface a lead whose status is closed/parked.
    ...
    // v15.11.24 — Exclude leads on THIS agent's holdout list. Both Skip and
  ...
  app.get("/api/admin/agent-stats", (req, res) => {
    ...
    // v15.11.24 — broaden the "green dot" signal (see /api/admin/leaderboard for full comment).
    ...
    const stats = allAgents.map(agent => {
      ...
      const contactRate = totalAttempts > 0
        ...
        : 0;
      // v15.11.24 — last-activity now considers outcomes, point-earning events, AND logins.
    ...
    // v15.11.24 — Unified sort: points (highest = #1), dials as tiebreaker.
    stats.sort((a, b) =>
...
{agentPhone} \u00b7 {agentEmail}`;
  ...
  // ─── RECYCLE LEAD ──────────────────────────────────────────────────────────
  // v15.11.24 — Helper: next midnight in America/New_York, returned as UTC ISO.
  ...
  // v15.11.24 — EDT midnight of TODAY (as UTC ISO). Used for the daily reset
  ...
  // v15.11.24 — Skip quota computation. Returns { used, remaining,
  ...
  // v15.11.24 — GET /api/agent/:id/skip-quota
  ...
  // v15.11.24 — POST /api/leads/:id/skip { agentId, notes? }
  ...
  app.post("/api/leads/:id/recycle", (req, res) => {
    ...
    // v15.11.24 — Anti-bounce-back: hold out from THIS agent for the rest of
  ...
  app.get("/api/admin/leaderboard", (req, res) => {
    ...
    // v15.11.24 — broaden the "green dot" signal. Was: only lead_activity outcomes.
    ...
    // v15.11.24 — Unified sort: points (highest = #1), then dials as tiebreaker.
...
<div style="max-width:580px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 28px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">
    Lead Depot v15.11.24 \u2014 Brothers Group \u00b7 Momentum Realty
  </div>
</div></body></html>`,
        ...
        if (resend) {
          try {
            ...
            await resend.emails.send({
              ...
              to: normEmail,
              subject: `${firstName}, your BGRE application — Lead Depot v15.11.24`,
              html,
              text: invitationBody,
...
<div style="max-width:680px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid rgba(200,170,90,0.25)">
  ...
  <div style="padding:16px 24px;margin-top:24px;background:#080808;border-top:1px solid rgba(255,255,255,0.05);font-size:11px;color:rgba(255,255,255,0.18);display:flex;justify-content:space-between">
    <span>Lead Depot v15.11.24</span><span>Brothers Group · Momentum Realty</span>
  </div>
</div>
</body>
</html>`;
