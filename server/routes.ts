...
<div style="max-width:580px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 28px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">Lead Depot v20.7.23 — Brothers Group · Momentum Realty</div>
</div></body></html>`;
...
<div style="max-width:620px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:14px 32px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444;display:flex;justify-content:space-between">
    <span>Lead Depot v20.7.23 — Brothers Group · Momentum Realty</span>
  </div>
</div>
</body>
</html>`;
...
<div style="max-width:500px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 26px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">Lead Depot v20.7.23 — Brothers Group · Momentum Realty</div>
</div></body></html>`,
    });
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family:Georgia,serif;background:#09090b;color:#e5e5e5;padding:40px 24px;max-width:600px;margin:0 auto;border-radius:12px;">
              ...
              <p style="color:#71717a;font-size:12px;margin-top:24px;">— Brothers Group Real Estate Team at Momentum Realty<br>Lead Depot v20.7.23</p>
            </div>
          `,
        });
      } catch (e: any) {
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family:'Georgia',serif;background:#09090b;color:#e5e5e5;padding:40px 24px;max-width:600px;margin:0 auto;border-radius:12px;">
              ...
              <p style="color:rgba(255,255,255,0.25);font-size:12px;line-height:1.6;border-top:1px solid rgba(200,170,90,0.1);padding-top:18px;">If you weren't expecting this reset, ignore this email — your password will not change. Lead Depot v20.7.23 · Brothers Group Real Estate Team at Momentum Realty</p>
            </div>
          `,
        });
        emailSent = true;
...
<div style="max-width:580px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid #2a2520">
  ...
  <div style="padding:12px 28px;background:#0a0908;border-top:1px solid #1e1c19;font-size:11px;color:#444">
    Lead Depot v20.7.23 \u2014 Brothers Group \u00b7 Momentum Realty
  </div>
</div></body></html>`,
...
<p style="margin:0;color:#666;font-size:13px">Alex Watson &middot; Brothers Group Real Estate at Momentum Realty</p>
  ...
  app.post("/api/admin/backfill-appt-points", async (_req: any, res: any) => {
    try {
      // Phase A: seed lead 5213 (Jose Ramos) into contacted_appointment for Bronson
      // if it's still sitting as keep_in_touch. Idempotent — skips if already flipped.
      ...
      if (ramos && ramos.status === "keep_in_touch") {
        rawDb.prepare(
          `UPDATE leads SET status = 'contacted_appointment', assigned_agent_id = 3, last_activity_at = ? WHERE id = 5213`
        ).run(new Date().toISOString());
        leadFlipped = true;
      }

      // Phase B: find every contacted_appointment lead that has an assigned agent
      // but is missing its matching agent_points row.
      const candidates = rawDb.prepare(`
        SELECT l.id AS lead_id, l.assigned_agent_id AS agent_id,
               COALESCE(l.last_activity_at, l.uploaded_at) AS ts,
      ...
      const tx = rawDb.transaction(() => {
        for (const c of candidates) {
          insertStmt.run(c.agent_id, 60, c.lead_id, c.ts);
          inserted.push({ agentId: c.agent_id, leadId: c.lead_id, ownerName: c.owner_name, address: c.address, ts: c.ts });
        }
      });
      tx();
...
<div style="max-width:680px;margin:0 auto;background:#0c0b0a;border-radius:14px;overflow:hidden;border:1px solid rgba(200,170,90,0.25)">
  ...
  <div style="padding:16px 24px;margin-top:24px;background:#080808;border-top:1px solid rgba(255,255,255,0.05);font-size:11px;color:rgba(255,255,255,0.18);display:flex;justify-content:space-between">
    <span>Lead Depot v20.7.23</span><span>Brothers Group · Momentum Realty</span>
  </div>
</div>
</body>
</html>`;
